...
def gee_multi_credentials(credentials_dir):
    credentials_files = glob.glob(os.path.join(credentials_dir, '*.json'))
    current_credentials_idx = 0

    def mpb_get_credentials_path():
        nonlocal current_credentials_idx
        while True:
            if current_credentials_idx >= len(credentials_files):
                current_credentials_idx = 0

            credential = credentials_files[current_credentials_idx]
            current_credentials_idx += 1

            print("Acessing GEE from %s" % credential)
            try:
                # Tenta autenticar com a credencial atual
                ee.Initialize(credentials=ee.oauth.Credentials.from_file(credential))
                return credential
            except Exception as e:
                print(f"Failed to authenticate with {credential}: {e}")
                if current_credentials_idx >= len(credentials_files):
                    raise Exception("All credentials failed to authenticate")
