    ...
    Points.csv = async function (request, response) {
        ...
        infoCampaign.find({'_id': campaign._id}).forEach(function (data) {
            pointsCollection.find({"campaign": campaign._id}).sort({'index': 1}).toArray(function (err, points) {
                // Linhas do CSV extraídas para serviço puro testável: legado
                // por ano (byte a byte igual ao algoritmo original) e Wayback
                // por célula da grade — ver services/supervisorCsv.js.
                var csvResult = supervisorCsv.buildCsvRows(points, data);
